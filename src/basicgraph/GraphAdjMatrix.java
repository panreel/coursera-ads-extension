...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		// define two hops distance list
		List<Integer> twoHopsDistances = new ArrayList<Integer>();

		// iterate over colIndex
		for (int colIndex = 0; colIndex < this.getNumVertices(); colIndex++)
		{
			...
			for (int rowIndex = 0; rowIndex < this.getNumVertices(); rowIndex++)
			{
				
				nOf2Hops += (adjMatrix[v][rowIndex] * adjMatrix[rowIndex][colIndex]);
				
			}
			
			// check if a 2 hops path exists - if yes add possible paths to distance list
			while(nOf2Hops > 0) {
				
				twoHopsDistances.add(colIndex);
				nOf2Hops--;
